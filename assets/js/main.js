...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        ...
        ably.connection.on('disconnected', () => {
            console.warn('Ably连接断开');
        });
        
        ably.connection.on('suspended', () => {
            console.error('Ably连接已挂起');
        });
...
function generateBookcases() {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        bookcase.className = 'bookcase slide-in';
        bookcase.dataset.id = i;
        bookcase.style.animationDelay = `${i * 0.1}s`;
...
function toggleNightMode() {
    ...
    if (nightModeBtn) {
        ...
        nightModeBtn.classList.add('animate-spin');
        setTimeout(() => {
            nightModeBtn.classList.remove('animate-spin');
        }, 1000);
    }
}
...
function toggleFullscreen() {
    ...
    if (!document.fullscreenElement) {
        viewerContainer.requestFullscreen().catch(err => {
            showError(`无法进入全屏模式: ${err.message}`);
        });
    } else {
...
function getComicsInBookcase(bookcaseId) {
    try {
        ...
        return files.map(file => ({
            name: file.name,
            url: file.directLink,
            format: file.format || (file.name.toLowerCase().endsWith('.pdf') ? 'pdf' : 'zip'),
